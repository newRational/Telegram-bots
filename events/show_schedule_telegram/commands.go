...
func (p *Processor) showSchedule(day string, username string, chatID int) error {
	daySchedule, err := p.readSchedule(day, username)
	if err != nil {
		return err
	}

	return p.tg.SendMessage(chatID, daySchedule)
}

func (p *Processor) readSchedule(day string, username string) (string, error) {
